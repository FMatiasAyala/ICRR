...
exports.modificacionEquipo = async (req, res) => {
  ...
  const query =
    "UPDATE tbl_equipomedico SET marca=?, modelo=?, serial_number=?, tipo=?, ip=?, mascara=?, gateway=?, aetitle=?, puerto=?, compra_año=?, id_ubicacion=?, alta=?, funcion=?, riesgo=?, requerimientos=?, antecedentes=? WHERE id=?";
